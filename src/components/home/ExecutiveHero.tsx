          ...
          <div className="relative backdrop-blur-md bg-white/5 border border-white/20 rounded-2xl p-5 md:p-6">
            <div className="flex items-center justify-between mb-3">
              <span className="inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full bg-lime-400/15 text-lime-200 border border-lime-400/20">
                <span>📌</span>
                <span>
                  {locale === 'en' ? 'Executive overview' : 'ภาพรวมหน่วยวิจัย'}
                </span>
              </span>
              {summary?.generated_at && (
                <span className="text-[10px] text-slate-400">
                  {locale === 'en' ? 'updated' : 'ปรับปรุง'}{' '}
            ...
            {/* Evidence disclosure (source/model strip hidden — internal use only) */}
            {hasChains && (
              <div className="mt-3 pt-3 border-t border-white/10 flex justify-end">
                <button
                  type="button"
                  onClick={() => setEvidenceOpen((v) => !v)}
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md border border-emerald-400/30 bg-emerald-400/10 text-emerald-200 hover:bg-emerald-400/20 transition text-[10px] font-medium"
        ...
        <EvidenceChainPanel
          ...
          intro={
            locale === 'en'
              ...
              : 'แต่ละข้อความในสรุปด้านบน อ้างอิงไปยังข้อมูลจริงในฐานข้อมูล CESRU (publications, patents, grants, innovations) — คลิกแต่ละข้อเพื่อดูที่มา'
          }
        />
      </section>
    )}
